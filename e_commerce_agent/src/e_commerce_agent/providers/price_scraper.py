    ...
    async def scrape_amazon(self, url: str) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if not price_text:
                     # Try price inside script tag (often used dynamically)
                    script_tag = soup.find('script', string=re.compile(r'"priceAmount"\s*:'))
                    if script_tag:
                        match = re.search(r'"priceAmount"\s*:\s*([\d\.]+)', script_tag.string)
                        if match:
                ...
                features = [bullet.get_text().strip() for bullet in feature_bullets if bullet.get_text().strip()]
